...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#AACCFF") {
        canvas = canvas_pass
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Ship{
        constructor(){
            this.count = 10
            this.center = new Circle(sea.poles[30].x, sea.poles[30].y, 10, "#FF0000")
            this.hull = new Polygon(this.center.x, this.center.y, 50, "B#AA8800", 5, 0, 0, Math.PI*.5, 1)
            this.pole = new Rectangle(this.center.x-5, this.center.y-450, 10, 450, "#885500")
            this.flag = new Rectangle(this.pole.x, this.pole.y, 40, 70, "#090909")
            this.nest = new Rectangle(this.pole.x-80, this.pole.y+90, 170, 90, "#885500")
            this.cannonpoint = new Point(this.center.x+100, this.center.y- 90)
            this.shots = []

        }
        draw(offset){
            this.count+=.5
            this.count%=sea.poles.length
            this.center = new Circle(200+offset, sea.poles[Math.floor(this.count)].y, 10, "#FF0000")
            this.pole = new Rectangle(this.center.x-5, this.center.y-450, 10, 450, "#885500")
            this.hull = new Polygon(this.center.x, this.center.y-20, 170, "#AA8800", 5, 0, 0, Math.PI*.5, 1)
            this.flag = new Rectangle(this.pole.x, this.pole.y, 70, 40, "#090909")
            this.nest = new Rectangle(this.pole.x-80, this.pole.y+90, 170, 90, "#885500")
            this.cannonpoint = new Point(this.center.x+200, this.center.y- 110)
            ...
            this.cannon = new LineOP(this.center, this.cannonpoint, "#444444", 30)
        ...
        fire(){
            let shot = new Circle(this.cannonpoint.x, this.cannonpoint.y, 14, "#111111", 7, -3.6)
    ...
    class Water{
        constructor(){
            ...
            this.angle = 0
            for(let t = 0;t<160;t++){
                let rect = new Rectangle(t*8, 500+(Math.cos(this.angle+(t*(Math.PI/20)))*12), 8, 1000, "#0055FF55")
                this.poles.push(rect)
            }
            for(let t = 0;t<160;t++){
                this.draw()
                this.tick--
        ...
        draw(){
            this.tick++
            for(let t = 0;t<this.poles.length;t++){
                this.poles[t].draw()
                if(this.tick%2 == 1){
                    this.poles[t].x-=8
                }
    ...
    function main() {
        ...
        friendship.draw(0)
        enemyship.draw(800)
        enemyship.count = Math.max(friendship.count-10,0)
        sea.draw()
    }
})
